...
fn test_utf8_bytes(string: &str) -> Result<(), EncodingError> {
...
fn test_utf8_vec(string: &str) -> Result<(), EncodingError> {
    let os_string = OsString::from(string.to_string());
    let string = string.as_bytes();
    assert_eq!(os_string, from_vec(string.to_vec())?);
...
#[test]
fn test_nonempty_utf8_bytes() -> Result<(), EncodingError> {
    test_utf8_bytes(UTF8_STRING)
}

#[test]
fn test_nonempty_utf8_vec() -> Result<(), EncodingError> {
